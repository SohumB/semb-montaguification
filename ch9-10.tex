...
\begin{itemize}
	\item \emph{Syntactic binding:}
	\begin{itemize}
		\item c-command and coindexation
		\item \Tree [ {every diver$_x$}  [ defended themselves$_x$ ] ]
		\item the DP \lq {every diver$_x$}\rq 
	\end{itemize}
	\item suppose: indices of pronouns (and the variables they get translated to) are present in surface syntax
	\item index required for semantic interpretability and precondition for movement
\end{itemize}

% \begin{exe}
% 	\ex \emph{Binding theory:}
% 	\begin{xlist}
% 		\ex Reflexive pronouns must be bound in their binding domain.
