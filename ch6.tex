...
\item Formally speaking, this requires that the restrictor of the quantifier not be empty: \\ $\lambda P_{et}.\lambda P^\prime_{et}.\exists x_e (P(x))  .  \forall x_e(P(x) -> P^\prime(x))$ 
...
\end{itemize}
\item \textbf{Solution}: The quantifier only presupposes that that a unicorn is 'mythologically possible,' not that it exists in the real world. 
\end{itemize}
